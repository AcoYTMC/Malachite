...
public interface MalachiteEntities {
    EntityTypeRegistrant ENTITIES = new EntityTypeRegistrant(Malachite.MOD_ID);

    EntityType<MalachiteDaggerEntity> MALACHITE_DAGGER = ENTITIES.register("malachite_dagger",
            EntityType.Builder.<MalachiteDaggerEntity>create(MalachiteDaggerEntity::new, SpawnGroup.MISC)
                    .dimensions(0.5f, 0.5f)
                    .maxTrackingRange(64)
                    .trackingTickInterval(20)
    );

    EntityType<EnergyBeamEntity> ENERGY_BEAM = ENTITIES.register("energy_beam",
            EntityType.Builder.<EnergyBeamEntity>create(EnergyBeamEntity::new, SpawnGroup.MISC)
                    .dimensions(EntityType.ARROW.getWidth(), EntityType.ARROW.getHeight())
                    .maxTrackingRange(64)
    );

    EntityType<EnergyOrbEntity> ENERGY_ORB = ENTITIES.register("energy_orb",
            EntityType.Builder.<EnergyOrbEntity>create(EnergyOrbEntity::new, SpawnGroup.MISC)
                    .dimensions(0.5F, 0.5F)
                    .maxTrackingRange(64)
                    .eyeHeight(0.25f)
    );
